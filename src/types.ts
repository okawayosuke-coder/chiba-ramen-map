...
export type SortKey = "rating" | "reviews" | "name" | "near" | "drive";
...
export interface Filters {
  ...
  sort: SortKey;
  /** 到達圏フィルタ（分）。null=絞り込まない。現在地から Isochrone でこの時間内に行ける店だけ表示。 */
  maxMin: number | null;
}
...
export const GENRE_DEFS: {
  ...
  // 醤油/魚介/背脂はスープのベースで店名に出にくいが、調査データ(overrides)で補完される
  { key: "shoyu", label: "醤油", kw: ["醤油", "正油", "しょうゆ"] },
  ...
  { key: "seabura", label: "背脂", kw: ["背脂", "背あぶら"] },
  { key: "gyokai", label: "魚介・濃厚", kw: ["魚介"] },
];
