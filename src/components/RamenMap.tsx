...
function FollowController({
  ...
  useEffect(() => {
    ...
    let prevSpeed: number | null = null; // 1つ前のフィックス速度（加減速の算出用・Phase2）
    ...
    const onFix = (p: GeolocationPosition) => {
      ...
      // Phase2: 前回フィックスからの経過秒（lastFixPerf更新前に取得）。加減速の算出に使う
      const dtFix = lastFixPerf > 0 ? (nowPerf - lastFixPerf) / 1000 : 0;
      if (drMode) {
...
function RamenMap({
  ...
  // 傾斜メーターのジャイロveto(Phase2)のON/OFF（設定トグル）を共有状態へ反映
  _pitch.enabled = gyroGrade;
